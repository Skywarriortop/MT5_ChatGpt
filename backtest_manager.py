...
import os 
import decimal 
import random # Untuk simulasi slippage dinamis
...
logger.setLevel(logging.DEBUG) # Tetap set DEBUG untuk log internal, tapi kita akan pakai print juga
...
class SimulatedTrade:
    def __init__(self, ticket: str, symbol: str, trade_type: str, price_open: Decimal,
                 ...
                 max_hold_duration: timedelta = None # New parameter
                ):
        ...
        self.slippage_points = slippage_points # This will be dynamic at execution, but stored for reference
        ...
        logger.debug(f"DEBUG SimulatedTrade.__init__: Risk Distance (points) = {float(risk_distance_points):.2f}")
        ...
        for i, tp_cfg in enumerate(tp_levels_config):
            ...
            if risk_distance_points > 0 and tp_cfg.get('price_multiplier') is not None: 
                ...
                logger.debug(f"DEBUG SimulatedTrade.__init__: TP{i+1} Target Profit (points) = {float(target_profit_points):.2f}")
            ...
            self.tp_levels[tp_key] = {
                ...
                'volume_ratio': utils.to_decimal_or_none(tp_cfg.get('volume_percentage_to_close', Decimal('1.00'))),
                'hit': False,
        ...
        self.max_hold_duration = max_hold_duration # Store the max hold duration
    ...
    def _calculate_profit_loss_for_volume(self, close_price: Decimal, volume_to_calc: Decimal, PIP_UNIT_IN_DOLLAR: Decimal) -> Decimal:
        ...
        # Selisih harga dalam unit harga (misal, $0.00050 untuk EURUSD)
        if self.type == 'buy':
            price_difference_units = (close_price - self.price_open)
        elif self.type == 'sell':
            price_difference_units = (self.price_open - close_price)
        
        # Konversi selisih harga (dalam unit harga) menjadi dolar
        # Jika self.point_value adalah 0.00001 (nilai 1 poin), dan pip_unit_in_dollar adalah nilai 1 pip dalam dolar.
        # Dan 1 pip = 10 poin.
        # P/L dalam Dolar = (price_difference_units / self.point_value) * (self.pip_unit_in_dollar / 10) * (volume_to_calc / Decimal('0.01'))
        profit_loss_currency = (price_difference_units / self.point_value) * (self.pip_unit_in_dollar / 10) * (volume_to_calc / Decimal('0.01'))
    ...
    def attempt_trail_stop_loss(self, trailing_stop_pips: Decimal, trailing_stop_step_pips: Decimal, pip_value_per_point: Decimal, current_price: Decimal):
        trailing_stop_points_value = trailing_stop_pips 
        trailing_stop_step_points_value = trailing_stop_step_pips 

        if trailing_stop_points_value <= 0 or trailing_stop_step_points_value <= 0 or self.sl_price is None:
            return False 
        ...
        if self.type == 'buy':
            if current_price > self.price_open + (trailing_stop_points_value * self.price_change_per_pip):
                new_sl_candidate = self.high_water_mark - (trailing_stop_points_value * self.price_change_per_pip)
                if new_sl_candidate > current_sl:
                    if (new_sl_candidate - current_sl) >= (trailing_stop_step_points_value * self.price_change_per_pip):
            ...
            if current_price < self.price_open - (trailing_stop_points_value * self.price_change_per_pip):
                new_sl_candidate = self.high_water_mark + (trailing_stop_points_value * self.price_change_per_pip)
                if new_sl_candidate < current_sl:
                    if (current_sl - new_sl_candidate) >= (trailing_stop_step_points_value * self.price_change_per_pip):
                        self.sl_price = new_sl_candidate.quantize(Decimal('0.00001'), rounding=decimal.ROUND_HALF_UP) 
    ...
    def attempt_breakeven(self):
        ...
        if move_to_be:
            # Check if current SL is worse than breakeven
    ...
    def check_and_execute_tp_levels(self, candle_high: Decimal, candle_low: Decimal, pip_unit_in_dollar: Decimal) -> tuple[str | None, Decimal | None]:
        ...
        # This prevents, e.g., tp2 being hit before tp1 logic is applied.
        for tp_key_order in ['tp1', 'tp2', 'tp3']: 
            ...
            if tp_info and tp_info['price'] is not None and not tp_info['hit'] and self.current_volume > Decimal('0.0000001'):
                ...
                if self.type == 'buy':
                    if candle_high >= tp_info['price']: # Hit jika candle high mencapai TP price atau lebih tinggi
                        tp_hit_condition = True
                elif self.type == 'sell':
                    if candle_low <= tp_info['price']: # Hit jika candle low mencapai TP price atau lebih rendah
                        tp_hit_condition = True
                
                if tp_hit_condition:
                    print(f"DEBUG_PRINT_BM: ST_TP: {self.ticket} {tp_key_order} HIT CONDITION TRUE. Price: {float(tp_info['price']):.5f}")
                    
                    # Calculate volume to close for this partial TP
                    volume_to_close = self.initial_volume * tp_info['volume_ratio'] 
                    
                    # Ensure we don't try to close more volume than currently open
                    if volume_to_close > self.current_volume:
                        volume_to_close = self.current_volume 
                        print(f"DEBUG_PRINT_BM: ST_TP: Adjusted volume to close to current volume: {float(volume_to_close):.5f}")
                    
                    if volume_to_close > Decimal('0.0000001'): # Ensure there's actual volume to close
                    ...
                    # Handle SL adjustment after partial close
                    if tp_info['move_sl_to_breakeven_after_partial'] and not self.breakeven_active:
                        ...
                        # Only move SL if it improves (makes it closer to entry or into profit)
    ...
    def check_sl_hit(self, candle_high: Decimal, candle_low: Decimal):
        ...
        if self.type == 'buy':
            if candle_low <= self.sl_price: # Hit jika candle low mencapai SL price atau lebih rendah
                sl_hit_condition = True
        elif self.type == 'sell':
            if candle_high >= self.sl_price: # Hit jika candle high mencapai SL price atau lebih tinggi
                sl_hit_condition = True
...
class BacktestManager:
    def __init__(self, symbol: str):
        ...
        self.open_positions: dict[str, SimulatedTrade] = {} # Mengelola multiple posisi
    ...
    def _simulate_trade_execution(self, current_candle: dict, signal_action: str, 
                                  ...
                                  tp1_sugg: Decimal, tp2_sugg: Decimal, tp3_sugg: Decimal,
                                  current_balance: Decimal): # Pass current_balance for risk management
        ...
        tp1_sugg_dec = utils.to_decimal_or_none(tp1_sugg)
        tp2_sugg_dec = utils.to_decimal_or_none(tp2_sugg)
        tp3_sugg_dec = utils.to_decimal_or_none(tp3_sugg)

        # Dynamic Spread and Slippage
        ...
        # Adjust SL based on actual entry price
        ...
        if sl_sugg_dec is not None and self.point_value > Decimal('0.0'):
            risk_per_trade_amount = current_balance * self.risk_percent_per_trade
            
            # Calculate risk per lot based on initial SL distance
            # Assuming 1 standard lot = 100,000 units for forex, so 1 lot = 100,000 * point_value
            # For simplicity, let's assume point_value is for 1 unit, so 1 lot has 100,000 units
            # Or simply, point_value * (1 standard lot / 0.01 lot) * (risk_distance in points)
            
            # Simplified risk calculation based on points for 1 unit, then scale by lot size
            ...
            if risk_points > Decimal('0.0'):
                ...
                # For simplicity, if point_value is already adjusted for standard lot, or if we use "points" as "pips" directly.
                # Let's use the definition: (price difference in points) * point_value * volume = P/L in USD.
                # So, volume = Risk_Amount / ((Risk_Points) * self.point_value)
                volume_from_risk = risk_per_trade_amount / (risk_points * self.point_value)
                
                # Ensure volume is at least minimum lot size (e.g., 0.01) and rounded
                min_lot_size = Decimal('0.01') # Typically 0.01 for micro lots
        ...
        simulated_position = SimulatedTrade(
            ...
            spread_points=actual_spread_points, # Store actual dynamic spread
            slippage_points=actual_slippage_points, # Store actual dynamic slippage
            point_value=self.point_value,
            max_hold_duration=self.max_position_hold_time # Pass max hold duration
        )
    ...
    def run_backtest(self, start_date: datetime, end_date: datetime,
        ...
        self.closed_trade_profits_losses = [] 
        self.simulated_trade_events = []
        self.open_positions: dict[str, SimulatedTrade] = {} # Initialize for each run
        ...
        local_closed_by_time_limit_count = 0 # New counter
        ...
        for i in range(len(df_candles)):
            ...
            if current_price_for_signal_dec is None:
                ...
                current_signal_data = None
                self.balance_history.append((current_time_for_signal, current_balance)) # Still record balance
                continue
            ...
            for ticket in list(self.open_positions.keys()): # Iterate over a copy to allow modification
                ...
                position.attempt_trail_stop_loss(
                    ...
                    self.pip_value_per_point, candle_close
                )
                ...
                if tp_hit_status:
                    ...
                    if tp_hit_status == 'CLOSED_FULLY':
                        ...
                        del self.open_positions[ticket] # Remove fully closed position
                ...
                if ticket in self.open_positions: # Check if still open after TP check
                ...
                # New: Check for max hold duration
            ...
            if action in ["BUY", "SELL"]: 
                ...
                sl_sugg_float = current_signal_data.get('stop_loss_suggestion')
                tp1_sugg_float = current_signal_data.get('take_profit_suggestion')
                tp2_sugg_float = current_signal_data.get('tp2_suggestion')
                tp3_sugg_float = current_signal_data.get('tp3_suggestion')

                sl_sugg = utils.to_decimal_or_none(sl_sugg_float)
                tp1_sugg = utils.to_decimal_or_none(tp1_sugg_float)
                tp2_sugg = utils.to_decimal_or_none(tp2_sugg_float)
                tp3_sugg = utils.to_decimal_or_none(tp3_sugg_float)
                ...
                if sl_sugg is None or tp1_sugg is None: 
                    ...
                    sl_distance_points = abs(current_price_for_signal_dec - sl_sugg) / self.point_value
                    tp_distance_points = abs(current_price_for_signal_dec - tp1_sugg) / self.point_value
                    print(f"DEBUG_PRINT_BM: Calculated SL Distance (points): {sl_distance_points:.2f}. Required: {float(min_sl_pips_dec):.2f}.")
                    print(f"DEBUG_PRINT_BM: Calculated TP1 Distance (points): {tp_distance_points:.2f}. Required: {float(min_tp_pips_dec):.2f}.")

                    if sl_distance_points < min_sl_pips_dec:
                        trade_rejection_reason = f"SL ({sl_distance_points:.2f} points) terlalu dekat."
                        print(f"DEBUG_PRINT_BM: TIDAK MEMBUKA POSISI: {trade_rejection_reason} (SL terlalu dekat).")
                    elif tp_distance_points < min_tp_pips_dec:
                        trade_rejection_reason = f"TP1 ({tp_distance_points:.2f} points) terlalu dekat."
                ...
                if can_attempt_trade_execution:
                    sim_pos = self._simulate_trade_execution(
                        ...
                        tp2_sugg,
                        tp3_sugg,
                        current_balance # Pass current balance for volume calculation
                    )
                    if sim_pos:
                        print(f"DEBUG_PRINT_BM: POSISI BERHASIL DIBUKA: {sim_pos.ticket}. Saldo: {float(current_balance):.2f}")
                        self.open_positions[sim_pos.ticket] = sim_pos # Add to open positions
                        if action == "BUY":
                            total_long_trades += 1
                        elif action == "SELL":
                ...
                print(f"DEBUG_PRINT_BM: TIDAK MEMBUKA POSISI: Sinyal bukan BUY/SELL atau tidak ada sinyal. Action: {action}.")

            self.balance_history.append((current_time_for_signal, current_balance)) # Record balance at end of candle
        ...
        # Calculate daily returns from balance history (after all trades are processed)
        # Ensure that balance points are for daily intervals
        df_balance_daily = df_balance.resample('D').last().ffill().dropna() # Get last balance of the day, forward fill missing days
        ...
        if not daily_returns_for_stats.empty and len(daily_returns_for_stats) > 1:
            risk_free_rate = Decimal('0.0')
            
            daily_returns_float = daily_returns_for_stats.apply(utils.to_float_or_none)
            daily_returns_float = daily_returns_float.dropna() 
            
            if len(daily_returns_float) > 1: 
                daily_std_dev = Decimal(str(daily_returns_float.std())) 
            else:
                daily_std_dev = Decimal('0.0') 

            if daily_std_dev > 0:
                sharpe_ratio = Decimal(str((daily_returns_float.mean() - float(risk_free_rate)) / float(daily_std_dev) * math.sqrt(252))) 
            
            negative_returns_float = daily_returns_float[daily_returns_float < 0]
            if len(negative_returns_float) > 0:
                downside_std_dev = Decimal(str(negative_returns_float.std())) 
                if downside_std_dev > 0:
                    sortino_ratio = Decimal(str((daily_returns_float.mean() - float(risk_free_rate)) / float(downside_std_dev) * math.sqrt(252))) 
        ...
        final_results = {
            ...
            "closed_by_time_limit_count": local_closed_by_time_limit_count, # New metric
    ...
    def _record_closed_trade_metrics(self, pnl: Decimal, exit_time: datetime, ticket: str, exit_price: Decimal, exit_reason: str,
        ...
        # This function is used to update the shared counters and lists
        # It's passed mutable objects like lists/dicts, or the values are reassigned by the caller.
        # This makes it modular but requires careful handling in the main loop.
        
        self.closed_trade_profits_losses.append(pnl)
        
        if is_full_close: # Only increment total trades for full closures
            ...
            if pnl > 0:
                ...
                consecutive_wins = 0
                largest_loss = min(largest_loss, pnl) # Store largest loss as negative
        ...
        # Return updated metrics to the caller to reassign local variables
    ...
    def _plot_results(self, balance_history: list, closed_trade_profits_losses: list, strategy_name: str, max_drawdown_value: Decimal) -> str:
        ...
        try:
            ...
            # Konversi kolom 'balance' ke float untuk plotting
            ...
            # Konversi closed_trade_profits_losses ke float untuk plotting
