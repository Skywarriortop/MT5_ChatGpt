...
def to_decimal_or_none(value):
    ...
    if isinstance(value, str):
        ...
        try:
            # Coba konversi string bersih. Ini bisa gagal jika ada karakter non-numerik lain
            return Decimal(cleaned_value)
        except (ValueError, TypeError, Exception):
            logger.warning(f"Gagal mengonversi '{value}' (tipe: {type(value)}) ke Decimal setelah membersihkan. Mengembalikan None.")
            return None

    # Tambahkan penanganan untuk float yang sangat kecil dan menjadi 0 di Decimal
    if isinstance(value, float):
        if math.isinf(value) or math.isnan(value): # Handle infinity and NaN floats
            return None
        return Decimal(str(value)) # Konversi melalui string untuk presisi

    try:
        return Decimal(value) # Coba konversi langsung jika bukan string/float
    except (ValueError, TypeError, Exception):
